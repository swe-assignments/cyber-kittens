...
exports.createOne = async (req, res, next) => {
  try {
    ...
    const created = await Kitten.create({ name, age, color });
    return res
      .status(201)
...
exports.deleteOne = async (req, res, next) => {
  try {
    const { id } = req.params;
    if (id != req.user.id) return res.status(401).send("Unauthorized");
    const one = await Kitten.destroy({ where: { id: id } });
    return res.status(204).send(one);
  } catch (error) {
    console.error("Kittens: getOne", error);
    next(error);
  }
